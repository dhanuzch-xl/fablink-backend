...
##
##This file is part of xlogic.
##
##xlogic is free software: you can redistribute it and/or modify
##it under the terms of the GNU Lesser General Public License as published by
##the Free Software Foundation, either version 3 of the License, or
##(at your option) any later version.
##
##xlogic is distributed in the hope that it will be useful,
##but WITHOUT ANY WARRANTY; without even the implied warranty of
##MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the
##GNU Lesser General Public License for more details.
##
##You should have received a copy of the GNU Lesser General Public License
##along with xlogic.  If not, see <http://www.gnu.org/licenses/>.
...
def calculate_shape_properties(shape):
    ...
    return {
        ...
        "Surface Area": f"{surface_area:.2f} mm²",
...
<head>
    <meta name="generator" content="xlogic-$VERSION X3D exporter (www.xlogiclabs.com)"/>
    <meta name="creator" content="xlogic-$VERSION generator"/>
    <meta name="identifier" content="http://www.xlogiclabs.com"/>
    <meta name="description" content="xlogic-$VERSION x3dom based shape rendering"/>
</head>
<Scene>
"""
)
...
HEADER_TEMPLATE = Template(
    """
<head>
    <title>xlogic $VERSION x3dom renderer</title>
    <meta name='Author' content='Thomas Paviot - tpaviot@gmail.com'>
    <meta name='Keywords' content='WebGl,xlogic'>
    <meta charset="utf-8">
...
<body>
    ...
    <div id="parts_list" style="position: absolute; left: 10px; top: 10px; background-color: rgba(255, 255, 255, 0.7); padding: 10px; border-radius: 5px; max-height: 80%; overflow-y: auto;">
    <h3>Parts List:</h3>
    <ul>
    $PARTS_LIST
    </ul>
    </div>
    <div id="selection_info" style="position: absolute; right: 10px; top: 10px; background-color: rgba(255, 255, 255, 0.7); padding: 10px; border-radius: 5px;">
    <h3>Selected Area:</h3>
    <p id="selected_area_info">No area selected</p>
    </div>
    <script>
    ...
    function highlightShape(shape) {
        if (highlighted_shape) {
            unhighlightShape(highlighted_shape);
        }
        highlighted_shape = shape;
        shape.getElementsByTagName('Material')[0].setAttribute('emissiveColor', '1 1 0');
    }

    function unhighlightShape(shape) {
        if (shape === selected_shape) return;
        shape.getElementsByTagName('Material')[0].setAttribute('emissiveColor', '0 0 0');
    }

    function selectShape(shape) {
        if (selected_shape) {
            selected_shape.getElementsByTagName('Material')[0].setAttribute('emissiveColor', '0 0 0');
        }
        selected_shape = shape;
        shape.getElementsByTagName('Material')[0].setAttribute('emissiveColor', '0 1 0');
        updateSelectionInfo(shape);
    }

    function updateSelectionInfo(shape) {
        var info = document.getElementById('selected_area_info');
        // You'll need to implement logic to determine if the shape is circular
        // and calculate its properties. For now, we'll just display the shape ID.
        info.textContent = 'Selected shape: ' + shape.getAttribute('DEF');
    }
    ...
    function fitCamera()
    {            
        var x3dElem = document.getElementById('xlogic-x3d-scene');            
...
class HTMLHeader:
    def __init__(self, bg_gradient_color1="#000000", bg_gradient_color2="#000000"):
...
class HTMLBody:
    def __init__(self, x3d_shapes, axes_plane, axes_plane_zoom_factor=1.0, part_names=None, part_properties=None):
        ...
        self._part_names = part_names or []
        self._part_properties = part_properties or {}

    def get_str(self):
        # get the location where xlogic is running from
        x3dcontent = "\n\t<x3d id='xlogic-x3d-scene' style='width:100%;height:100%;border:none' >\n\t\t<Scene>\n"
        ...
        for part in self._part_names:
            parts_list_html += f"<li><strong>{part}</strong><ul>"
            if part in self._part_properties:
                for prop, value in self._part_properties[part].items():
                    parts_list_html += f"<li>{prop}: {value}</li>"
            parts_list_html += "</ul></li>"

        return BODY_TEMPLATE.substitute(
            {"VERSION": f"{VERSION}", "X3DSCENE": f"{x3dcontent}", "PARTS_LIST": parts_list_html}
        )
...
class X3DExporter:
    ...
    def to_x3dfile_string(self, shape_id):
        ...
        for i, triangle_set in enumerate(self._triangle_sets):
            x3dfile_str += "<Switch whichChoice='0' id='swBRP'>"
            x3dfile_str += f"<Transform scale='1 1 1'>\n<Shape DEF='shape{shape_id}_{i}' onclick='selectShape(this);' onmouseover='highlightShape(this);' onmouseout='unhighlightShape(this);'>\n"
    ...
    # Add a method to detect circular areas
    def detect_circular_areas(self):
        # This is a placeholder. You'll need to implement the actual detection logic
        # based on your specific requirements and the geometry of your shapes.
        # For now, we'll just return an empty list.
        return []


class XLogicX3DomRenderer:
    def __init__(self, path=None, display_axes_plane=True, axes_plane_zoom_factor=1.0):
        ...
        self._part_names = []  # New attribute to store part names
        self._part_properties = {}  # New attribute to store part properties

        print(
            f"## xlogic x3dom webgl renderer - render axes/planes : {self._axes_plane} - axes/plane zoom factor : {self._axes_plane_zoom_factor}"
        )
    ...
    def DisplayShape(
        ...
        circular_areas = x3d_exporter.detect_circular_areas()
        
        # Add circular areas as separate entities
        for i, area in enumerate(circular_areas):
            area_hash = f"circ{shape_uuid}_{i}"
            # You'll need to implement the logic to create X3D content for circular areas
            x3d_content = self.create_circular_area_x3d(area)
            area_filename = os.path.join(self._path, f"{area_hash}.x3d")
            with open(area_filename, 'w') as f:
                f.write(x3d_content)
            self._x3d_shapes[area_hash] = [True, color, specular_color, shininess, transparency, line_color, line_width]
        ...
        if part_name:
            ...
            self._part_properties[part_name] = calculate_shape_properties(shape)
    ...
    def generate_html_file(self, axes_plane, axes_plane_zoom_factor):
        ...
        with open(self._html_filename, "w") as html_file:
            ...
            html_file.write(
                HTMLBody(all_shapes, axes_plane, axes_plane_zoom_factor, self._part_names, self._part_properties).get_str()
            )
            html_file.write("</html>\n")

    def create_circular_area_x3d(self, area):
        # Implement the logic to create X3D content for a circular area
        # This is a placeholder and will depend on how you represent circular areas
        return f"<Shape><Cylinder radius='{area['radius']}' height='0.1'/></Shape>"
