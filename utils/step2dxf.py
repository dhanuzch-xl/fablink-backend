...
def get_face_normal(face):
    """Calculate and return the normal vector of a face."""
    surf = BRepAdaptor_Surface(face)
    return surf.Plane().Axis().Direction()


def rotate_vertical_faces(faces):
    """
    Rotates vertical faces to flatten them for projection on the XZ plane.
    """
    for face in faces:
        normal = get_face_normal(face)
        # Check if the normal is vertical (aligned with Y-axis)
        if abs(normal.Dot(gp_Dir(0, 1, 0))) > 0.99:
            # Apply a rotation to align the face with the XZ plane
            transformation = gp_Trsf()
            transformation.SetRotation(gp_Ax1(gp_Pnt(0, 0, 0), gp_Dir(0, 1, 0)), math.pi / 2)
